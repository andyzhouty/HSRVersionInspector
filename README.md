...
直接运行 `uv run hvi` 时，会进入导航菜单。选择“查看数据”或“比较版本差异”后，
向导会依次让你选择版本、模式和资源编号；所有选择都输入数字即可。直接运行
`uv run hvi show` 或 `uv run hvi diff` 也会进入对应向导，按 `q` 返回或退出。

`list`、`show` 和 `diff` 支持 `--markdown`，可配合 `>` 保存或转发。`show 版本`
...
`download-all`. The same command also downloads the source-driven enemy
...
`download-all` reads every `version` entry from `versionID.json` as the URL
version segment, then downloads the `character`, `lightcone`, `maze`, `story`,
`boss`, and `peak` resources into `data/{version}/zh/{mode}/{id}.json`.
...
remaining downloads. Enemy HP is calculated from the downloaded
